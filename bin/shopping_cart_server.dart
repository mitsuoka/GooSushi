/*
  ...
  Usage:
    1) Run this shopping_cart_server.dart.
    ...
    HttpServer.bind(HOST, PORT).then((HttpServer server) {
      ...
      server.listen((HttpRequest request) {
        request.response.done.then((d) {
          ...
          print(
              "${new DateTime.now()} : Error occured while sending response: $e");
        });
...
class ShoppingCart {
  ...
  void set orderedAt(DateTime time) {
    _orderedAt = time;
  }
...
class ShoppingCartItem {
  ...
  void set itemCode(int itemCode) {
    _itemCode = itemCode;
  }

  double get perItemCost => _perItemCost;

  void set perItemCost(double perItemCost) {
    _perItemCost = perItemCost;
  }

  int get iQty => _qty;

  void set iQty(int iQty) {
    _qty = iQty;
  }

  String get itemName => _itemName;

  void set itemName(String itemName) {
    _itemName = itemName;
  }

  double get subTotal => _subTotal;

  void set subTotal(double subTotal) {
    _subTotal = subTotal;
  }
}
...
<html>
  ...
  CARTBASE.sortedItemCodes().forEach((itemCode) {
    ...
    if (cart == null || cart.getCartItem(MENU[itemCode].itemCode) == null) {
      text2 =
          "<option>0<option>1<option>2<option>3<option>4<option>5</select></td></tr>";
    } else {
...
request.headers :
  ...
  if (request.method == "POST") {
    ...
    if (enctype[0].contains("text")) {
      ...
      sb.write(
          "request body string (URL decoded): ${Uri.decodeFull(bodyString)}");
    }
  }
  sb.write("\n");
  return sb;
